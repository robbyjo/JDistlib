 ...
 */
package net.sourceforge.jdistlib;

import net.sourceforge.jdistlib.generic.GenericDistribution;
import net.sourceforge.jdistlib.math.Constants;
import net.sourceforge.jdistlib.math.MathFunctions;
...
public class Tweedie extends GenericDistribution  {
	...
	public static final double dlogfdphi(double y, double mu, double phi, double xi) {
		...
		// if (p > 2)
		double kv = 0, dv_dphi = 0, logv = 0;
		if (y >= 1) {
			kv = dtweedie_kv_bigp(y, phi, xi);
			dv_dphi = (kv * (a - 1))/phi;
			logv = dtweedie_logv_bigp(y, phi, xi);
		}

		if (y < 1 || Double.isInfinite(logv) || Double.isNaN(logv)) {
	...
	private static final double dtweedie_jw_smallp(double y, double phi, double xi) {
		...
		double og = 0, ologj = 0;
		int nj = (int) (hi_j - lo_j + 1);
		double[] A = new double[nj];
		for (int j = lo_j ; j <= hi_j ; j++) {
			og += MathFunctions.lgammafn( j+1 ) + MathFunctions.lgammafn( -a*j );
			ologj += log(j);
			A[(int) (j - lo_j)] = r * j;
		}
	...
	private static final double dtweedie_kv_bigp(double y, double phi, double xi) {
		...
		double a = ( 2 - xi ) / ( 1 - xi ),
			a1 = 1 - a,
			r = -a*log(y) + a*log(xi-1) - a1*log(phi) - log(xi-2);

		int[] range = find_range(y, phi, xi, r);
		int lo_k = range[0], hi_k = range[1];

		// Now sum the series between established limits.
		// We ensure it works for vector y.
		double og = 0, ologk = 0, sumC = 0;
		int nk = (int) (hi_k - lo_k + 1);
		double[] A = new double[nk];
		for (int k = lo_k ; k <= hi_k ; k++) {
			og += MathFunctions.lgammafn( 1+a*k ) - MathFunctions.lgammafn( 1+k );
			ologk += log(k);
			A[(int) (k - lo_k)] = r * k;
			sumC += sin( -a*PI*k ) * (k % 2 == 0 ? 1 : -1);
		}
		double m = Double.NEGATIVE_INFINITY;
		for (int i = 0; i < nk; i++) {
			A[i] = A[i] + og + ologk;
	...
	public static final double dtweedie_series(double y, double mu, double phi, double xi, boolean give_log) {
		...
		// Special cases
		if (xi == 1) return Poisson.density(y/phi, mu/phi, give_log);
		if (xi == 2) return Gamma.density(y, 1/phi, 1/(phi * mu), give_log);
		if (y == 0) {
	...
	private static final double smallp(double y, double mu, double phi, double p, double aimrerr) {
		int m = -1, iteratn = 0, sbuffer = 0, flag = 0, totalits = 1, ier = 0, tier = 0;
		double area = 0, area0 = 0, area1 = 0, relerr = 1, resultp = 0, zero1 = 0;
		double[] wold = new double[] {0, 0, 0};
		// TODO
		return 0;
	}

	/**
	//  Calculates the density of the log-likelihood function of a Poisson-Gamma distribution by inverting the moment generating function.
	//
	//**
	//     IN:   p, phi, y, mu, exact
	...
	//             : 0 to keep quiet
	//**/
	private static final double pdf(double y, double mu, double phi, double p, boolean exact) {
		if (p == 1)
			return -10;
		if (p > 1 && p < 2) {
			if (y < 0) return 0;
			return exp(-calclambda(mu, phi, p));
		}
		if (p >= 2.0) {
			if (y <= 0) return 0;
		}
		int maxit = 100, m = -1, iteratn = 0, ier = 0;
		double aimrerr = 1e-10, area = 0, relerr = 1.0, savemu = mu, result = 0;

		if (exact) {
			if (p > 1 && p <= 2) {
				// FIXME smallp(p, phi, y, mu);
			} else if (p > 2) {
				// TODO bigp
			}
		} else {
