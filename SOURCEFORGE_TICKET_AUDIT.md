...
| [#12](https://sourceforge.net/p/jdistlib/tickets/12/) | [#5](https://github.com/robbyjo/JDistlib/issues/5) | R PR#15554, Bessel functions | Completed. `Bessel.j` and `Bessel.y` reject unrepresentable orders before integer conversion or allocation, the exact `2^64` cases return `NaN`, and 0.8.1 corrects the previously lost connection term for negative half-integer orders. Representable small-argument/large-order values and legitimate binary64 underflow have focused coverage. |
