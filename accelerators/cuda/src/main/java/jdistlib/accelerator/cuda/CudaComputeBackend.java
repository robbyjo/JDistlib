...
public final class CudaComputeBackend implements ComputeBackend {
	...
	private static final String SOURCE =
			...
			+ "for(int j=0;j<input;j++)s+=(tr?a[j*cols+i]:a[i*cols+j])*x[j];y[i]=(alpha==0?0:alpha*s)+(beta==0?0:beta*y[i]);}}"
			...
			+ "for(int q=0;q<k;q++)s+=(ta?a[q*m+row]:a[row*k+q])*(tb?b[col*k+q]:b[q*n+col]);int z=row*n+col;c[z]=(alpha==0?0:alpha*s)+(beta==0?0:beta*c[z]);}}"
			+ "extern \"C\" __global__ void blas_syrk(int tr,int n,int k,double alpha,const double*a,double beta,double*c){int row=blockIdx.y*blockDim.y+threadIdx.y,col=blockIdx.x*blockDim.x+threadIdx.x;if(row<n&&col<n){double s=0;for(int q=0;q<k;q++)s+=(tr?a[q*n+row]:a[row*k+q])*(tr?a[q*n+col]:a[col*k+q]);c[row*n+col]=(alpha==0?0:alpha*s)+(beta==0?0:beta*c[row*n+col]);}}"
			...
			+ "int row=blockIdx.x*blockDim.x+threadIdx.x;if(row<rows){double s=0;for(int z=rs[row]-1;z<rs[row+1]-1;z++)s+=v[z]*x[ci[z]-1];y[row]=(alpha==0?0:alpha*s)+(beta==0?0:beta*y[row]);}}"
			...
			+ "for(int q=rs[row]-1;q<rs[row+1]-1;q++)s+=v[q]*b[(ci[q]-1)*outcols+col];c[z]=(alpha==0?0:alpha*s)+(beta==0?0:beta*c[z]);}}"
			...
			+ "int row=blockIdx.x*blockDim.x+threadIdx.x;if(row<rows){float s=0;for(int z=rs[row]-1;z<rs[row+1]-1;z++)s+=v[z]*x[ci[z]-1];y[row]=(alpha==0?0:alpha*s)+(beta==0?0:beta*y[row]);}}"
	...
	@Override public synchronized SymmetricEigenDecomposition dsyev(double[] matrix,int dimension){checkDecompositionMatrix(matrix,dimension,dimension);matrix=matrix.clone();double matrixScale=normalizeDecomposition(matrix);checkSymmetric(matrix,dimension);ensureAvailable();setCurrent();CUdeviceptr work=allocate(matrix),values=allocateDoubles(dimension),vectors=allocateDoubles(matrix.length),info=allocateInts(1);try{launch("decomp_dsyev",1,1,1,1,1,1,Pointer.to(Pointer.to(work),Pointer.to(values),Pointer.to(vectors),Pointer.to(new int[]{dimension}),Pointer.to(new double[]{16*Math.ulp(1.0)}),Pointer.to(info)));if(copyInts(info,1)[0]!=0)throw new IllegalStateException("CUDA symmetric eigendecomposition did not converge");return new SymmetricEigenDecomposition(dimension,rescaleDecomposition(copy(values,dimension),matrixScale),copy(vectors,matrix.length));}finally{cuMemFree(info);cuMemFree(vectors);cuMemFree(values);cuMemFree(work);}}
	@Override public synchronized SingularValueDecomposition dgesvd(double[] matrix,int rows,int columns){checkDecompositionMatrix(matrix,rows,columns);matrix=matrix.clone();double matrixScale=normalizeDecomposition(matrix);ensureAvailable();setCurrent();int count=Math.min(rows,columns);CUdeviceptr a=allocate(matrix),work=allocateDoubles(matrix.length),u=allocateDoubles(rows*count),singular=allocateDoubles(count),vt=allocateDoubles(count*columns),info=allocateInts(1);try{launch("decomp_dgesvd",1,1,1,1,1,1,Pointer.to(Pointer.to(a),Pointer.to(work),Pointer.to(u),Pointer.to(singular),Pointer.to(vt),Pointer.to(new int[]{rows}),Pointer.to(new int[]{columns}),Pointer.to(new double[]{16*Math.ulp(1.0)}),Pointer.to(info)));if(copyInts(info,1)[0]!=0)throw new IllegalStateException("CUDA SVD did not converge");return new SingularValueDecomposition(rows,columns,rescaleDecomposition(copy(singular,count),matrixScale),copy(u,rows*count),copy(vt,count*columns));}finally{cuMemFree(info);cuMemFree(vt);cuMemFree(singular);cuMemFree(u);cuMemFree(work);cuMemFree(a);}}
	...
	@Override public synchronized FloatSymmetricEigenDecomposition ssyev(float[] matrix,int dimension){checkDecompositionMatrix(matrix,dimension,dimension);matrix=matrix.clone();float matrixScale=normalizeDecomposition(matrix);checkSymmetric(matrix,dimension);ensureAvailable();setCurrent();CUdeviceptr work=allocate(matrix),values=allocateFloats(dimension),vectors=allocateFloats(matrix.length),info=allocateInts(1);try{launch("decomp_ssyev",1,1,1,1,1,1,Pointer.to(Pointer.to(work),Pointer.to(values),Pointer.to(vectors),Pointer.to(new int[]{dimension}),Pointer.to(new float[]{16*Math.ulp(1.0f)}),Pointer.to(info)));if(copyInts(info,1)[0]!=0)throw new IllegalStateException("CUDA FP32 symmetric eigendecomposition did not converge");return new FloatSymmetricEigenDecomposition(dimension,rescaleDecomposition(copyFloats(values,dimension),matrixScale),copyFloats(vectors,matrix.length));}finally{cuMemFree(info);cuMemFree(vectors);cuMemFree(values);cuMemFree(work);}}
	@Override public synchronized FloatSingularValueDecomposition sgesvd(float[] matrix,int rows,int columns){checkDecompositionMatrix(matrix,rows,columns);matrix=matrix.clone();float matrixScale=normalizeDecomposition(matrix);ensureAvailable();setCurrent();int count=Math.min(rows,columns);CUdeviceptr a=allocate(matrix),work=allocateFloats(matrix.length),u=allocateFloats(rows*count),singular=allocateFloats(count),vt=allocateFloats(count*columns),info=allocateInts(1);try{launch("decomp_sgesvd",1,1,1,1,1,1,Pointer.to(Pointer.to(a),Pointer.to(work),Pointer.to(u),Pointer.to(singular),Pointer.to(vt),Pointer.to(new int[]{rows}),Pointer.to(new int[]{columns}),Pointer.to(new float[]{16*Math.ulp(1.0f)}),Pointer.to(info)));if(copyInts(info,1)[0]!=0)throw new IllegalStateException("CUDA FP32 SVD did not converge");return new FloatSingularValueDecomposition(rows,columns,rescaleDecomposition(copyFloats(singular,count),matrixScale),copyFloats(u,rows*count),copyFloats(vt,count*columns));}finally{cuMemFree(info);cuMemFree(vt);cuMemFree(singular);cuMemFree(u);cuMemFree(work);cuMemFree(a);}}
