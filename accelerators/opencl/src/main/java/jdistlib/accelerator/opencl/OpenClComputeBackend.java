...
public final class OpenClComputeBackend implements ComputeBackend {
	...
	private static final String SOURCE =
			...
			+ "__kernel void blas_gemv(int tr,int rows,int cols,double alpha,__global const double*a,__global const double*x,double beta,__global double*y){int i=get_global_id(0),output=tr?cols:rows,input=tr?rows:cols;if(i<output){double s=0;for(int j=0;j<input;j++)s+=(tr?a[j*cols+i]:a[i*cols+j])*x[j];y[i]=(alpha==0?0:alpha*s)+(beta==0?0:beta*y[i]);}}\n"
			+ "__kernel void blas_gemm(int ta,int tb,int m,int n,int k,double alpha,__global const double*a,__global const double*b,double beta,__global double*c){int row=get_global_id(1),col=get_global_id(0);if(row<m&&col<n){double s=0;for(int q=0;q<k;q++)s+=(ta?a[q*m+row]:a[row*k+q])*(tb?b[col*k+q]:b[q*n+col]);int z=row*n+col;c[z]=(alpha==0?0:alpha*s)+(beta==0?0:beta*c[z]);}}\n"
			+ "__kernel void blas_syrk(int tr,int n,int k,double alpha,__global const double*a,double beta,__global double*c){int row=get_global_id(1),col=get_global_id(0);if(row<n&&col<n){double s=0;for(int q=0;q<k;q++)s+=(tr?a[q*n+row]:a[row*k+q])*(tr?a[q*n+col]:a[col*k+q]);c[row*n+col]=(alpha==0?0:alpha*s)+(beta==0?0:beta*c[row*n+col]);}}\n"
			...
			+ "__kernel void csr_mv(int rows,double alpha,__global const double*v,__global const int*ci,__global const int*rs,__global const double*x,double beta,__global double*y){int row=get_global_id(0);if(row<rows){double s=0;for(int z=rs[row]-1;z<rs[row+1]-1;z++)s+=v[z]*x[ci[z]-1];y[row]=(alpha==0?0:alpha*s)+(beta==0?0:beta*y[row]);}}\n"
			+ "__kernel void csr_mm(int rows,int outcols,double alpha,__global const double*v,__global const int*ci,__global const int*rs,__global const double*b,double beta,__global double*c){int z=get_global_id(0);if(z<rows*outcols){int row=z/outcols,col=z-row*outcols;double s=0;for(int q=rs[row]-1;q<rs[row+1]-1;q++)s+=v[q]*b[(ci[q]-1)*outcols+col];c[z]=(alpha==0?0:alpha*s)+(beta==0?0:beta*c[z]);}}\n"
			...
			+ "__kernel void float_blas_gemv(int tr,int rows,int cols,float alpha,__global const float*a,__global const float*x,float beta,__global float*y){int i=get_global_id(0),output=tr?cols:rows,input=tr?rows:cols;if(i<output){float s=0;for(int j=0;j<input;j++)s+=(tr?a[j*cols+i]:a[i*cols+j])*x[j];y[i]=(alpha==0?0:alpha*s)+(beta==0?0:beta*y[i]);}}\n"
			+ "__kernel void float_blas_gemm(int ta,int tb,int m,int n,int k,float alpha,__global const float*a,__global const float*b,float beta,__global float*c){int row=get_global_id(1),col=get_global_id(0);if(row<m&&col<n){float s=0;for(int q=0;q<k;q++)s+=(ta?a[q*m+row]:a[row*k+q])*(tb?b[col*k+q]:b[q*n+col]);int z=row*n+col;c[z]=(alpha==0?0:alpha*s)+(beta==0?0:beta*c[z]);}}\n"
			+ "__kernel void float_blas_syrk(int tr,int n,int k,float alpha,__global const float*a,float beta,__global float*c){int row=get_global_id(1),col=get_global_id(0);if(row<n&&col<n){float s=0;for(int q=0;q<k;q++)s+=(tr?a[q*n+row]:a[row*k+q])*(tr?a[q*n+col]:a[col*k+q]);c[row*n+col]=(alpha==0?0:alpha*s)+(beta==0?0:beta*c[row*n+col]);}}\n"
			...
			+ "__kernel void float_csr_mv(int rows,float alpha,__global const float*v,__global const int*ci,__global const int*rs,__global const float*x,float beta,__global float*y){int row=get_global_id(0);if(row<rows){float s=0;for(int z=rs[row]-1;z<rs[row+1]-1;z++)s+=v[z]*x[ci[z]-1];y[row]=(alpha==0?0:alpha*s)+(beta==0?0:beta*y[row]);}}\n"
			+ "__kernel void float_csr_mm(int rows,int outcols,float alpha,__global const float*v,__global const int*ci,__global const int*rs,__global const float*b,float beta,__global float*c){int z=get_global_id(0);if(z<rows*outcols){int row=z/outcols,col=z-row*outcols;float s=0;for(int q=rs[row]-1;q<rs[row+1]-1;q++)s+=v[q]*b[(ci[q]-1)*outcols+col];c[z]=(alpha==0?0:alpha*s)+(beta==0?0:beta*c[z]);}}\n"
	...
	@Override public synchronized SymmetricEigenDecomposition dsyev(double[]matrix,int dimension){checkDecompositionMatrix(matrix,dimension,dimension);matrix=matrix.clone();double matrixScale=normalizeDecomposition(matrix);checkSymmetric(matrix,dimension);ensureAvailable();cl_mem work=inputReadWrite(matrix),values=output(dimension),vectors=output(matrix.length),info=outputInts(1);cl_kernel operation=kernel("decomp_dsyev");try{arg(operation,0,work);arg(operation,1,values);arg(operation,2,vectors);arg(operation,3,dimension);arg(operation,4,16*Math.ulp(1.0));arg(operation,5,info);run1d(operation,1);if(readInts(info,1)[0]!=0)throw new IllegalStateException("OpenCL symmetric eigendecomposition did not converge");return new SymmetricEigenDecomposition(dimension,rescaleDecomposition(read(values,dimension),matrixScale),read(vectors,matrix.length));}finally{clReleaseKernel(operation);clReleaseMemObject(info);clReleaseMemObject(vectors);clReleaseMemObject(values);clReleaseMemObject(work);}}
	@Override public synchronized SingularValueDecomposition dgesvd(double[]matrix,int rows,int columns){checkDecompositionMatrix(matrix,rows,columns);matrix=matrix.clone();double matrixScale=normalizeDecomposition(matrix);ensureAvailable();int count=Math.min(rows,columns);cl_mem a=input(matrix),work=output(matrix.length),u=output(rows*count),singular=output(count),vt=output(count*columns),info=outputInts(1);cl_kernel operation=kernel("decomp_dgesvd");try{arg(operation,0,a);arg(operation,1,work);arg(operation,2,u);arg(operation,3,singular);arg(operation,4,vt);arg(operation,5,rows);arg(operation,6,columns);arg(operation,7,16*Math.ulp(1.0));arg(operation,8,info);run1d(operation,1);if(readInts(info,1)[0]!=0)throw new IllegalStateException("OpenCL SVD did not converge");return new SingularValueDecomposition(rows,columns,rescaleDecomposition(read(singular,count),matrixScale),read(u,rows*count),read(vt,count*columns));}finally{clReleaseKernel(operation);clReleaseMemObject(info);clReleaseMemObject(vt);clReleaseMemObject(singular);clReleaseMemObject(u);clReleaseMemObject(work);clReleaseMemObject(a);}}
	...
	@Override public synchronized FloatSymmetricEigenDecomposition ssyev(float[]matrix,int dimension){checkDecompositionMatrix(matrix,dimension,dimension);matrix=matrix.clone();float matrixScale=normalizeDecomposition(matrix);checkSymmetric(matrix,dimension);ensureAvailable();cl_mem work=inputReadWrite(matrix),values=outputFloats(dimension),vectors=outputFloats(matrix.length),info=outputInts(1);cl_kernel operation=kernel("decomp_ssyev");try{arg(operation,0,work);arg(operation,1,values);arg(operation,2,vectors);arg(operation,3,dimension);arg(operation,4,16*Math.ulp(1.0f));arg(operation,5,info);run1d(operation,1);if(readInts(info,1)[0]!=0)throw new IllegalStateException("OpenCL FP32 symmetric eigendecomposition did not converge");return new FloatSymmetricEigenDecomposition(dimension,rescaleDecomposition(readFloats(values,dimension),matrixScale),readFloats(vectors,matrix.length));}finally{clReleaseKernel(operation);clReleaseMemObject(info);clReleaseMemObject(vectors);clReleaseMemObject(values);clReleaseMemObject(work);}}
	@Override public synchronized FloatSingularValueDecomposition sgesvd(float[]matrix,int rows,int columns){checkDecompositionMatrix(matrix,rows,columns);matrix=matrix.clone();float matrixScale=normalizeDecomposition(matrix);ensureAvailable();int count=Math.min(rows,columns);cl_mem a=input(matrix),work=outputFloats(matrix.length),u=outputFloats(rows*count),singular=outputFloats(count),vt=outputFloats(count*columns),info=outputInts(1);cl_kernel operation=kernel("decomp_sgesvd");try{arg(operation,0,a);arg(operation,1,work);arg(operation,2,u);arg(operation,3,singular);arg(operation,4,vt);arg(operation,5,rows);arg(operation,6,columns);arg(operation,7,16*Math.ulp(1.0f));arg(operation,8,info);run1d(operation,1);if(readInts(info,1)[0]!=0)throw new IllegalStateException("OpenCL FP32 SVD did not converge");return new FloatSingularValueDecomposition(rows,columns,rescaleDecomposition(readFloats(singular,count),matrixScale),readFloats(u,rows*count),readFloats(vt,count*columns));}finally{clReleaseKernel(operation);clReleaseMemObject(info);clReleaseMemObject(vt);clReleaseMemObject(singular);clReleaseMemObject(u);clReleaseMemObject(work);clReleaseMemObject(a);}}
