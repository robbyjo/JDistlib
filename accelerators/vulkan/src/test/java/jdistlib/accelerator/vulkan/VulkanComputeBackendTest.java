...
public class VulkanComputeBackendTest {
	@Test public void vulkanServiceCanBeRequiredExplicitly() {
		...
		try (ComputeSelection selection = ComputeBackends.select(Compute.VULKAN)) {
			...
			assertEquals(ComputeApi.VULKAN, selection.deviceInfo().api());
			assertTrue(!"unknown".equals(selection.deviceInfo().apiVersion()));
		}
	}
	...
	@Test public void preparedSparseFactorsRemainInVulkanBuffers(){VulkanComputeBackend backend=new VulkanComputeBackend();Assume.assumeTrue(backend.available());try{
		CsrMatrix matrix=new CsrMatrix(2,2,new double[]{4,1,3},new int[]{1,1,2},new int[]{1,2,4});
		try(jdistlib.accelerator.PreparedSparseCholesky factor=backend.prepareDcsrpotrf(matrix,MatrixTriangle.LOWER)){assertArrayEquals(new double[]{1,2},factor.solve(new double[]{6,7}),3e-12);assertEquals(Math.log(11),factor.logDeterminant(),3e-12);boolean rejected=false;try{factor.refactor(new CsrMatrix(2,2,new double[]{1,2,1},new int[]{1,1,2},new int[]{1,2,4}));}catch(IllegalArgumentException expected){rejected=true;}assertTrue(rejected);assertArrayEquals(new double[]{1,2},factor.solve(new double[]{6,7}),3e-12);factor.refactor(new CsrMatrix(2,2,new double[]{5,1,4},new int[]{1,1,2},new int[]{1,2,4}));assertArrayEquals(new double[]{1,2},factor.solve(new double[]{7,9}),3e-12);}
		CsrMatrix fill=new CsrMatrix(3,3,new double[]{4,1,3,1,3},new int[]{1,1,2,1,3},new int[]{1,2,4,6});
		try(jdistlib.accelerator.PreparedSparseCholesky factor=backend.prepareDcsrpotrf(fill,MatrixTriangle.LOWER,jdistlib.accelerator.SparseOrdering.NATURAL)){assertArrayEquals(new double[]{1,4,2,5,3,6},factor.solve(new double[]{9,27,7,19,10,22},2),4e-12);assertEquals(6,factor.factorNonzeroCount());}
		FloatCsrMatrix floats=new FloatCsrMatrix(2,2,new float[]{4,1,3},new int[]{1,1,2},new int[]{1,2,4});
		try(jdistlib.accelerator.PreparedFloatSparseCholesky factor=backend.prepareScsrpotrf(floats,MatrixTriangle.LOWER)){assertArrayEquals(new float[]{1,2},factor.solve(new float[]{6,7}),4e-5f);}
		assertTrue(backend.capabilities().nativeSparseFactorizations());
	}finally{backend.close();}}
	@Test public void decompositionsPreserveExtremeGlobalScale() {
		VulkanComputeBackend backend=new VulkanComputeBackend();Assume.assumeTrue(backend.available());
		try {
