...
public class Binomial extends GenericDistribution {
	...
	public static final double density_raw(double x, double n, double p, double q, boolean log_p)
	{
		...
		if (x == 0) {
			...
			if (p > q)
				return log_p ? n * log(q) : pow(q, n);
			return log_p ? n * log1p(-p) : pow1p(-p, n);
		}
		if (x == n) {
			if (p > q)
				return log_p ? n * log1p(-q) : pow1p(-q, n);
			return log_p ? n * log(p) : pow(p, n);
		}
		if (x < 0 || x > n) return (log_p ? Double.NEGATIVE_INFINITY : 0.);
		if (isInfinite(n)) {
			if (!isInfinite(x)) return log_p ? Double.NEGATIVE_INFINITY : 0;
			n = Double.MAX_VALUE;
		}
	...
	public static final double quantile(double p, double n, double pr, boolean lower_tail, boolean log_p)
	{
		...
		final double nn = n;
		return DiscreteQuantile.quantile(p, lower_tail, log_p, mu, sigma, gamma, n,
				(value, lt, lp) -> cumulative(value, nn, pr, lt, lp));
	}
