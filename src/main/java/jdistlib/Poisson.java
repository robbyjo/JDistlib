...
public class Poisson extends GenericDistribution implements SupportedDistribution,
	...
	public static class RandomState {
		...
		double pp[] = new double[36], p0, p, q, s, d, omega;
	...
	public static final double density(double x, double lambda, boolean give_log) {
		...
		// R_forceint keeps an integer-valued double as a double. Math.round would
		// saturate values above Long.MAX_VALUE and corrupt extreme-tail results.
		return density_raw(rint(x), lambda, give_log) ;
	}
	...
	public static final double quantile(double p, double lambda, boolean lower_tail, boolean log_p) {
		...
		return DiscreteQuantile.quantile(p, lower_tail, log_p, mu, sigma, gamma,
				...
				(value, lt, lp) -> cumulative(value, lambda, lt, lp));
	}
