...
/**
 * <P>Translated from <a href="https://cran.r-project.org/web/packages/tweedie/">Tweedie package</a>
 * version 2.2.1, dated 2014-06-06, by Roby Joehanes
 * 
 * <P>Parameterization: mu = mean, phi = dispersion, xi = power.
 * 
 * <P>See <a href="https://en.wikipedia.org/wiki/Tweedie_distribution">Wikipedia</a> for details.
 * 
 * @author Peter K. Dunn
 *
 */
public class Tweedie extends GenericDistribution  {
	private static String
		errXi = "Tweedie distribution is undefined for xi < 0 or for 0 < xi < 1",
		errXi12 = "This routine requires 1 < xi < 2",
		errXi2 = "This routine requires xi > 2",
		errPhi = "Tweedie distribution requires phi > 0",
		errMuPos = "This routine requires mu > 0",
		errYPos = "This routine requires y > 0",
		errYNonNeg = "This routine requires y >= 0",
		errY = "Tweedie distribution requires y > 0 for xi >= 2, y => 0 for 1 < xi < 2. y < 0 is only allowable for xi == 0";

	public static final double deviance(double y, double mu, double xi) {
		if (xi < 0 || (xi > 0 && xi < 1)) throw new RuntimeException(errXi);
		if (mu <= 0) throw new RuntimeException(errMuPos);
		double dev;
		if (xi == 0) {
			dev = y-mu;
			return dev * dev;
			...
			return 2 * (y == 0 ? mu : y * log(y / mu) - (y - mu));
		} else if (xi == 2) {
			return 2 * (log(mu) - log(y) + y/mu - 1);
		}
		// RJ Note: may suffer from severe underflow.
		return 2 * ( pow(y, (2 - xi)) / ((1 - xi) * (2 - xi)) - 
      	(y * pow(mu, 1 - xi)) /(1 - xi) + 
			pow(mu, 2 - xi) / (2 - xi) );
	}

	public static final double loglik(double y, double mu, double phi, double xi) {
		return density(y, mu, phi, xi, true);
	}

	/**
	 * Calculates d(log f)/d(phi) for the Tweedie densities.
	 * It is used, for example, in mle fitting of phi.  We would then sum over y and set this function to 0.
	 * @param y
	 * @param mu
	 * @param phi
	 * @param xi
	 * @return
	 */
	public static final double dlogfdphi(double y, double mu, double phi, double xi) {
		if (xi < 0 || (xi > 0 && xi < 1)) throw new RuntimeException(errXi);
		validateParameters(mu, phi, xi);
		double phiSq = phi * phi;
		if (xi == 0) {
			double residual = y - mu;
			return -0.5 / phi + residual * residual / (2 * phiSq);
		}
		if (xi == 1) {
			return (mu - y - y*log(mu/phi) + y*PolyGamma.digamma(1+(y/phi))) / phiSq;
		}
		if (xi == 2) {
			return (-log(y)  + ( y/mu ) + PolyGamma.digamma(1/phi) - 1 + log( mu*phi )) / phiSq;
		}
		double
			a = (2 - xi) / (1 - xi),
			A = (y * pow(mu, 1 - xi))/(phiSq * (xi - 1)),
			B = pow(mu, 2 - xi)/(phiSq * (2 - xi));

		if (xi > 1 && xi < 2) {
			if (y <= 0) return pow(mu, 2-xi) / ( phiSq * (2-xi) );
			double jw = dtweedie_jw_smallp(y, phi, xi);
			double dw_dphi = (jw * (a - 1)) / phi;
			double logw = dtweedie_logw_smallp(y, phi, xi);
			return A + B + dw_dphi/exp(logw);
		}
		// The alternating big-p series can lose all significant digits in its
		// analytic derivative.  Differentiate the completed log density instead.
		double step = cbrt(Math.ulp(1.0)) * max(1.0, phi);
		double lo = max(phi - step, phi * 0.5);
		double hi = phi + step;
		return (loglik(y, mu, hi, xi) - loglik(y, mu, lo, xi)) / (hi - lo);
	}

	private static void validateParameters(double mu, double phi, double xi) {
		if (Double.isNaN(mu) || Double.isNaN(phi) || Double.isNaN(xi))
			throw new RuntimeException("Tweedie parameters must not be NaN");
		if (xi < 0 || (xi > 0 && xi < 1)) throw new RuntimeException(errXi);
		if (mu <= 0 || Double.isInfinite(mu)) throw new RuntimeException(errMuPos);
		if (phi <= 0 || Double.isInfinite(phi)) throw new RuntimeException(errPhi);
	}

	private static double probabilityResult(double lower, boolean lowerTail, boolean logP) {
		lower = min(1.0, max(0.0, lower));
		if (lowerTail)
			return logP ? log(lower) : lower;
		double upper = 1.0 - lower;
		return logP ? (lower > 0.5 ? log1p(-lower) : log(upper)) : upper;
	}

	private static double inputProbability(double p, boolean lowerTail, boolean logP) {
		if ((logP && p > 0) || (!logP && (p < 0 || p > 1))) return Double.NaN;
		double probability = logP ? exp(p) : p;
		return lowerTail ? probability : 1.0 - probability;
	}

	private static final int[] find_range(double y, double phi, double xi, double r) {
		double a = ( 2 - xi ) / ( 1 - xi ),
			a1 = 1 - a,
		...
		double logz = r, // max(r), // To find largest  j  needed
				j_max = pow(y, 2-xi) / ( phi * (xi < 2 ? 2-xi : xi-2) ), // max(pow(y, 2-p) / ( phi * (2-p) )), // R variables are vector based. Not applicable here
				j = max( 1, j_max ),
				c = logz + a1 + a*log(-a),
				wmax = a1*j_max,
				estlogw = wmax;
		...
		double lo_j = max(1, floor(j));
		return new int[] { (int) lo_j, (int) hi_j };
	}

	private static final double dtweedie_jw_smallp(double y, double phi, double xi) {
		if (xi < 1 || xi > 2) throw new RuntimeException(errXi12);
		if (phi <= 0) throw new RuntimeException(errPhi);
		if (y <= 0) throw new RuntimeException(errYPos);
		double a = ( 2 - xi ) / ( 1 - xi ),
			a1 = 1 - a,
			r = -a*log(y) + a*log(xi-1) - a1*log(phi) - log(2-xi);
		int[] range = find_range(y, phi, xi, r);
		int lo_j = range[0], hi_j = range[1];
		...
		for (int j = lo_j ; j <= hi_j ; j++) {
			A[j - lo_j] = r * j - MathFunctions.lgammafn(j + 1)
					- MathFunctions.lgammafn(-a * j) + log(j);
		}
	...
	private static final double dtweedie_logw_smallp(double y, double phi, double xi) {
		if (xi < 1 || xi > 2) throw new RuntimeException(errXi12);
		if (phi <= 0) throw new RuntimeException(errPhi);
		if (y <= 0) throw new RuntimeException(errYPos);
	...
	private static final int[] findRangeBigP(double y, double phi, double xi, double r) {
		double a = (2 - xi) / (1 - xi);
		double a1 = 1 - a;
		double kMax = pow(y, 2 - xi) / (phi * (xi - 2));
		double k = max(1, kMax);
		double c = r + a1 + a * log(a);
		double vmax = a1 * kMax;
		double estimate = vmax;
		while (estimate > vmax - 37) {
			k += 2;
			estimate = k * (c - a1 * log(k));
		}
		int hi = (int) ceil(k);
		k = max(1, kMax);
		estimate = vmax;
		while (estimate > vmax - 37 && k >= 2) {
			k = max(1, k - 2);
			estimate = k * (c - a1 * log(k));
		}
		return new int[] {(int) max(1, floor(k)), hi};
	}

	private static double bigPSeries(double y, double phi, double xi, boolean weightedByK) {
		double a = (2 - xi) / (1 - xi);
		double a1 = 1 - a;
		double r = -a1 * log(phi) - log(xi - 2) - a * log(y) + a * log(xi - 1);
		int[] range = findRangeBigP(y, phi, xi, r);
		double maximum = Double.NEGATIVE_INFINITY;
		for (int k = range[0]; k <= range[1]; k++) {
			double coefficient = sin(-a * PI * k);
			if (coefficient != 0) {
				double term = r * k + MathFunctions.lgammafn(1 + a * k)
						- MathFunctions.lgammafn(1 + k) + log(abs(coefficient));
				if (weightedByK) term += log(k);
				maximum = max(maximum, term);
			}
		}
		if (!Double.isFinite(maximum)) return Double.NaN;
		double sum = 0;
		...
		for (int k = range[0]; k <= range[1]; k++) {
			...
			double term = r * k + MathFunctions.lgammafn(1 + a * k)
					- MathFunctions.lgammafn(1 + k) + log(abs(coefficient));
			if (weightedByK) term += log(k);
			double value = copySign(exp(term - maximum), coefficient);
			double corrected = value - compensation;
			double updated = sum + corrected;
			compensation = (updated - sum) - corrected;
			sum = updated;
		}
		return sum > 0 ? log(sum) + maximum : Double.NaN;
	}

	private static final double dtweedie_kv_bigp(double y, double phi, double xi) {
		if (xi < 2) throw new RuntimeException(errXi2);
		if (phi <= 0) throw new RuntimeException(errPhi);
		if (y <= 0) throw new RuntimeException(errYPos);

		return exp(bigPSeries(y, phi, xi, true));
	}

	private static final double dtweedie_logv_bigp(double y, double phi, double xi) {
		if (xi < 2) throw new RuntimeException(errXi2);
		if (phi <= 0) throw new RuntimeException(errPhi);
		if (y <= 0) throw new RuntimeException(errYPos);

		return bigPSeries(y, phi, xi, false);
	}

	public static final double dtweedie_series(double y, double mu, double phi, double xi, boolean give_log) {
		if (xi < 1) throw new RuntimeException(errXi2);
		if (phi <= 0) throw new RuntimeException(errPhi);
		if (mu <= 0) throw new RuntimeException(errMuPos);
		if (y < 0) throw new RuntimeException(errYNonNeg);

		// Special cases
		if (xi == 1) {
			double value = Poisson.density(y / phi, mu / phi, give_log);
			return give_log ? value - log(phi) : value / phi;
		}
		if (xi == 2) return Gamma.density(y, 1 / phi, phi * mu, give_log);
		if (xi == 3) return InvNormal.density(y, mu, sqrt(phi), give_log);
		if (y == 0) {
			if (xi > 2) return give_log ? Double.NEGATIVE_INFINITY : 0;
			if (xi > 1 && xi < 2) {
				double lambda = pow(mu, 2 - xi) / (phi * (2 - xi));
				return give_log ? -lambda : exp(-lambda);
			}
		}
		if (xi > 2) {
			// dtweedie.series.bigp
			double logv = dtweedie_logv_bigp(y, phi, xi);
			double theta = pow(mu, 1-xi) / ( 1 - xi );
			double kappa = pow(mu, 2-xi) / ( 2 - xi );

			double logfnew = (y*theta-kappa)/phi - log( PI*y) + logv;
			return give_log ? logfnew : exp(logfnew);
		}

		// dtweedie.series.smallp
		double logw = dtweedie_logw_smallp(y, phi, xi);
		double tau = phi * (xi-1) * pow(mu, xi-1);
		double lambda = pow(mu, 2-xi ) / ( phi * ( 2-xi ) );
		double logf = -y/tau - lambda - log(y) + logw;
		return give_log ? logf : exp(logf);
	}
	...
	public static final double dtweedie_saddle(double y, double mu, double phi, double xi, double eps, boolean give_log) {
		...
		if (xi == 0) return Normal.density(y, mu, sqrt(phi), give_log);
		if (y == 0) {
			if (xi >= 1 && xi < 2) {
				double lambda = -pow(mu, 2 - xi)/(phi * (2 - xi));
				return give_log ? lambda : exp(lambda);
		...
		double y_eps = xi < 2 ? y + eps : y;
		double dev = deviance(y, mu, xi);
		//double density = pow(2*PI*phi*pow(y_eps,xi), -0.5) * exp( -dev/(2*phi) );
		double density = -0.5*( Constants.M_LN_2PI + log(phi) + xi * log(y_eps) ) - dev/(2*phi);
		return give_log ? density : exp(density);
	}
	...
	//
	 */
	private static final double[] calccgf(double p, double phi, double y, double x) {
		/*
		 ...
		 * then the mgf of the conditional distribution, M*, is given by
		 * 
		 *           M(t) - exp( -lambda )
		 *  M*(t) = ----------------------
		 *            1 - exp( -lambda )
		 ...
		 *  K*(t) = log M* = log( exp( cgf ) - exp( -lambda ) ) - log( 1.0 - exp( -lambda ) )
		 */
		double psi = atan((1 - p) * x * phi);
		double alpha = (2.0e00 - p) / (1.0e00 - p);
		double front = 1.0e00 / (phi * (2.0e00 - p));
		double denom = pow(cos(psi), alpha);
		double im = front * sin(psi * alpha) / denom - x * y;
		double rl = front * cos(psi * alpha) / denom - front;
		return new double[] {rl, im};
	}

	private static final double imgcgf(double p, double phi, double y, double x) {
		double psi = atan((1 - p) * x * phi);
		double alpha = (2.0e00 - p) / (1.0e00 - p);
		double front = 1.0e00 / (phi * (2.0e00 - p));
		double denom = pow(cos(psi), alpha);
	...
	private static final double f(double p, double phi, double y, double x) {
		double psi = atan((1 - p) * x * phi);
		double alpha = (2.0e00 - p) / (1.0e00 - p);
		double front = 1.0e00 / (phi * (2.0e00 - p));
		double denom = pow(cos(psi), alpha);
		double im = front * sin(psi * alpha) / denom - x * y;
		double rl = front * cos(psi * alpha) / denom - front; // Same to calccgf up to this point
		return exp(rl) * cos(im);
	}

	private static final double calclambda(double mu, double phi, double xi) {
		return pow(mu, 2 - xi) / (phi * (2 - xi));
	}

	private static final double f2(double p, double phi, double y, double mu, double x) {
		if (x == 0) return 1;
		double psi = atan((1 - p) * x * phi);
		double alpha = (2.0e00 - p) / (1.0e00 - p);
		double front = 1.0e00 / (phi * (2.0e00 - p));
		...
		double im = front * sin(psi * alpha) / denom - x * y;
		double rl = front * cos(psi * alpha) / denom - front; // Same to calccgf up to this point
		double lambda = calclambda(mu, phi, p);
	...
	public double density(double x, boolean log) {
		return density(x, mu, phi, xi, log);
	}

	@Override
	public double cumulative(double p, boolean lower_tail, boolean log_p) {
		return cumulative(p, mu, phi, xi, lower_tail, log_p);
	}

	@Override
	public double quantile(double q, boolean lower_tail, boolean log_p) {
		return quantile(q, mu, phi, xi, lower_tail, log_p);
	}

	@Override
	public double random() {
		return random(mu, phi, xi, random);
	}
}
