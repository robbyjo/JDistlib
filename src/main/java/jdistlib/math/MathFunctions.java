...
public class MathFunctions {
	...
	public static final double stirlerr(double n)
	{
		...
		if (n <= 5.25) {
			if (n >= 1) {
				double logN = log(n);
				return lgammafn(n) + n * (1 - logN) + 0.5 * (logN - M_LN_2PI);
			}
		...
		if (n <= 23.5) {
			if (n > 12.8) last = 6;
			else if (n > 12.3) last = 7;
			else if (n > 8.9) last = 8;
			else if (n > 7.3) last = 10;
			else if (n > 6.6) last = 12;
			else if (n > 6.1) last = 14;
			else last = 16;
		} else {
			if (n > 15.7e6) last = 0;
			else if (n > 6180) last = 1;
			else if (n > 205) last = 2;
			else if (n > 86) last = 3;
			else if (n > 27) last = 4;
			else last = 5;
		}
	...
	public static final double bd0(double x, double np)
	{
		...
		if (abs(x-np) < 0.1*(x+np)) {
			...
			v = d / (x + np);
			if (d != 0 && v == 0) {
				double scaledX = ldexp(x, -2);
				double scaledNp = ldexp(np, -2);
				v = (scaledX - scaledNp) / (scaledX + scaledNp);
			}
			s = 0.5 * d * v;
			if (abs(2 * s) < DBL_MIN)
				return 2 * s;
		...
		double logRatio = isInfinite(x / np) ? log(x) - log(np) : log(x / np);
	...
	public static final double pow1p(double x, double y) {
		...
		double onePlusX = x + 1;
		double recoveredX = onePlusX - 1;
		return recoveredX == x || abs(x) > 0.5 || Double.isNaN(x)
				? pow(onePlusX, y) : exp(y * log1p(x));
	}
