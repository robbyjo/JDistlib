...
public class VectorMath {
	...
	public static final boolean isEqualScaled(double a, double b, double tol) {
		return (Double.isNaN(a) && Double.isNaN(b)) ||
				(a == b || abs(a - b) <= tol * Math.max(1.0, Math.max(abs(a), abs(b))));
	}
	...
	public static final boolean allEqual(double[] a, double[] b, double tol) {
		...
		for (int i = 0; i < a.length; i++) {
			...
			difference += abs(a[i] - b[i]) / a.length;
			scale += abs(a[i]) / a.length;
		}
		return scale > tol ? difference / scale <= tol : difference <= tol;
	}
